...
df_log["Month"] = df_log["date"].apply(lambda x: x.month); df_log["Month"] = df_log["Month"].map("{:02}".format)
df_log["Day"] = df_log["date"].apply(lambda x: x.day); df_log["Day"] = df_log["Day"].map("{:02}".format)
df_log["Date"] = None
...
# 統計量を計算する関数を定義
def CalculateTable(df, ind, colname=None, method=None):
...
def CalculateEachTemperature(df, ind, colname1=None, colname2=None, method=None):
    ...
    for i in range(len(ind)-1):
        ...
        if method == "sum":
            tmp_daytime = []
            tmp_night = []
            for row in range(start_flag, end_flag+1):
                if df_ex[colname2][row] != 0:
                    tmp_daytime.append(df_ex[colname1][row])
                else:
                    tmp_night.append(df_ex[colname1][row])
...
# 結果の出力
B_temperature_sum_daytime, B_temperature_sum_night = CalculateEachTemperature(df_log, ind_not_nan, colname1="ハウス内データ-温度1",
                                                                              colname2="ハウス内推定日射量", method="sum")
